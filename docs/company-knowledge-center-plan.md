...
已落地首个可运行切片：上传后的单文档可按类型解析为独立 Markdown 正文，并优先复用系统 LLM API 配置生成待审核知识派生物；管理员也可显式选择服务器 Codex CLI 作为生成器。当前支持 PDF、DOCX、PPTX、XLSX 与 UTF-8 文本/Markdown/常见代码格式，前端可在原文件、解析正文、AI 提炼三种视图间切换。

本切片仍是 P0 同步处理版本：派生物状态固定为 `draft`，不会覆盖原文件，也不会自动进入已发布知识；异步队列、结构化双阶段编译、人工审核发布、增量缓存与融合检索仍按下文阶段推进。
