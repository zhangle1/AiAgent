...
3. **第一阶段应复用当前浏览器内的 `ChatStreamProvider`，但不能把它当成长期运行事实来源。** 当前流状态刷新即丢失，后续必须增加可恢复的 Run/Event 账本，再让聊天页与画布共同消费。
...
1. 用户打开会话节点的聊天弹窗，历史仍从会话接口读取。
2. 发送动作调用与聊天页相同的 Chat Composer/stream interface；不另建“画布发送 API”。
...
桌面端采用“左侧画布/过滤 + 中央无限画布 + 右侧检查器”。聊天、文件和 diff 可用可调整大小的模态窗口承载；同一时间建议最多一个主模态，避免窗口管理取代工作本身。节点只呈现摘要，不在卡片内部嵌入完整滚动聊天。
...
交付：个人画布、session/project/note/group 节点、布局保存、语义边、筛选/小地图/检查器、原聊天页深链；运行态桥接 `ChatStreamProvider`。
...
2. 同页聊天弹窗首期是否必须可发送，还是只读详情 + 原页跳转即可？建议 Phase 1 只读，Phase 3 再发送。
