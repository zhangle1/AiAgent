...
public sealed class ChatSessionService : IChatSessionService
{
    ...
    private static string SerializePreferences(ChatCompleteRequest request) => JsonSerializer.Serialize(new { knowledge_base_names = request.KnowledgeBaseNames, code_project_id = request.CodeProjectId, code_repository_names = request.CodeRepositoryNames, model_id = request.ModelId, mode = request.Mode });
