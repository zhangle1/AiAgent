...
function EditUserDialog({ user, projects, onClose, onSaved }: { user: AdminUser; projects: CodeProject[]; onClose: () => void; onSaved: (user: AdminUser, message: string) => Promise<void> }) {
  const [alias, setAlias] = useState(user.alias || ""); const [projectIds, setProjectIds] = useState(user.project_ids); const [canCommitCode, setCanCommitCode] = useState(user.can_commit_code); const [error, setError] = useState(""); const [saving, setSaving] = useState(false);
  const submit = async (event: React.FormEvent) => { event.preventDefault(); setSaving(true); setError(""); try { await updateAdminUserAlias(user.id, alias); if (user.role !== "admin") { await updateAdminUserProjects(user.id, projectIds); await updateAdminUserCodeCommitPermission(user.id, canCommitCode); } await onSaved({ ...user, alias: alias.trim() || null, project_ids: user.role === "admin" ? user.project_ids : projectIds, can_commit_code: user.role === "admin" || canCommitCode }, "用户信息已保存。"); } catch (reason) { setError(reason instanceof Error ? reason.message : "保存用户失败。"); } finally { setSaving(false); } };
