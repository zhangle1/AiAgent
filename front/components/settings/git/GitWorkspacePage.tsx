...
import { ChevronDown, ChevronRight, Download, File, FileDiff, Folder, FolderGit2, FolderOpen, GitBranch, Loader2, RefreshCw, RotateCcw, Upload } from "lucide-react";
...
import type { CodeProject, CodeRepository, GitDiffComparison, GitWorkspaceBranches, GitWorkspaceDiff, GitWorkspaceDiffFile, GitWorkspaceStatus } from "@/lib/code-repository-types";
...
type GitDiffTreeNode = { children: Map<string, GitDiffTreeNode>; file?: GitWorkspaceDiffFile };

function GitDiffWorkspaceFiles({ diff, loading }: { diff: GitWorkspaceDiff | null; loading: boolean }) {
  const tree = useMemo(() => buildGitDiffTree(diff?.files ?? []), [diff?.files]);
  if (loading) return <div className="grid h-52 place-items-center p-3 text-xs text-slate-400"><Loader2 size={16} className="mr-2 inline animate-spin"/>正在生成 Diff…</div>;
  if (!diff) return <div className="grid h-52 place-items-center p-3 text-xs text-slate-400">正在加载文件变更…</div>;
  return <div className="grid min-h-[260px] grid-cols-[minmax(190px,0.42fr)_minmax(0,1fr)]"><aside className="workspace-scroll min-h-0 overflow-auto border-r border-slate-200 bg-slate-50/60 p-2"><div className="mb-2 px-1 text-[11px] font-semibold text-slate-500">文件变更</div>{diff.files.length ? <GitDiffTreeNodeView node={tree}/> : <p className="px-2 py-5 text-center text-xs leading-5 text-slate-400">暂无文件变更。</p>}</aside><div className="min-w-0 p-3">{diff.content ? <pre className="workspace-scroll max-h-[420px] overflow-auto rounded-lg bg-slate-950 p-4 font-mono text-xs leading-5 text-slate-100">{diff.content}</pre> : <p className="rounded-lg bg-slate-50 px-4 py-8 text-center text-xs text-slate-500">{diff.files.length ? "文件树已列出变更；未跟踪的新增文件在 Git 中尚无可对比基线。" : diff.message || "暂无差异。"}</p>}{diff.is_truncated ? <p className="mt-2 text-[11px] text-amber-700">Diff 内容较长，当前仅显示前 240,000 个字符。</p> : null}</div></div>;
}

function buildGitDiffTree(files: GitWorkspaceDiffFile[]) {
  const root: GitDiffTreeNode = { children: new Map() };
...
function GitDiffTreeNodeView({ node, depth = 0 }: { node: GitDiffTreeNode; depth?: number }) {
  return <div>{[...node.children.entries()].sort(([leftName, left], [rightName, right]) => Number(Boolean(left.file)) - Number(Boolean(right.file)) || leftName.localeCompare(rightName)).map(([name, child]) => <GitDiffTreeItem key={name} name={name} node={child} depth={depth}/>)}</div>;
}

function GitDiffTreeItem({ name, node, depth }: { name: string; node: GitDiffTreeNode; depth: number }) {
  const [expanded, setExpanded] = useState(depth < 2);
  const hasChildren = node.children.size > 0;
  if (node.file && !hasChildren) return <div title={node.file.old_path ? `${node.file.old_path} → ${node.file.path}` : node.file.path} className="flex min-h-8 items-center gap-1.5 rounded-md py-1 pr-2 text-xs text-slate-700" style={{ paddingLeft: `${depth * 12 + 8}px` }}><File size={13} className="shrink-0 text-slate-400"/><span className="min-w-0 flex-1 truncate">{name}</span><GitDiffStatus status={node.file.status}/></div>;
  return <div><button type="button" onClick={() => setExpanded((current) => !current)} className="flex min-h-8 w-full items-center gap-1 rounded-md py-1 pr-2 text-left text-xs font-medium text-slate-700 hover:bg-slate-100" style={{ paddingLeft: `${depth * 12 + 4}px` }}><span className="grid h-4 w-4 place-items-center">{expanded ? <ChevronDown size={13}/> : <ChevronRight size={13}/>}</span>{expanded ? <FolderOpen size={14} className="shrink-0 text-amber-500"/> : <Folder size={14} className="shrink-0 text-amber-500"/>}<span className="min-w-0 truncate">{name}</span></button>{expanded && <GitDiffTreeNodeView node={node} depth={depth + 1}/>}</div>;
}

function GitDiffStatus({ status }: { status: string }) {
  const code = status === "??" ? "U" : status[0]?.toUpperCase() || "M";
  const label = code === "A" || code === "U" ? "新增" : code === "D" ? "删除" : code === "R" ? "重命名" : code === "C" ? "复制" : "修改";
  const color = code === "A" || code === "U" ? "bg-emerald-50 text-emerald-700" : code === "D" ? "bg-rose-50 text-rose-700" : code === "R" ? "bg-violet-50 text-violet-700" : "bg-amber-50 text-amber-700";
  return <span title={label} className={`rounded px-1.5 py-0.5 text-[10px] font-semibold ${color}`}>{code}</span>;
}
