...
function FileSelection({ title, empty, items, selected, onToggle }: { title: string; empty: string; items: string[]; selected: string[]; onToggle: (value: string) => void }) { return <div className="mt-4"><span className="text-xs font-medium text-slate-700">{title}</span>{items.length ? <div className="mt-2 grid gap-1 sm:grid-cols-2">{items.map((item) => <label key={item} className="flex min-w-0 cursor-pointer items-center gap-2 rounded-md bg-white px-2 py-1.5 text-xs text-slate-600"><input type="checkbox" checked={selected.includes(item)} onChange={() => onToggle(item)} className="rounded border-slate-300 text-blue-600 focus:ring-blue-500"/><span className="truncate font-mono" title={item}>{item}</span></label>)}</div> : <p className="mt-2 text-xs text-slate-400">{empty}</p>}</div>; }
