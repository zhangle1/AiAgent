...
type CloneDraft = { projectId: number | ""; repositoryUrl: string; destinationDirectoryName: string; branch: string; gitAccountId: number | "" };
...
const languages = ["C#", "TypeScript/JavaScript", "Java", "Python"];
...
const emptyClone: CloneDraft = { projectId: "", repositoryUrl: "", destinationDirectoryName: "", branch: "", gitAccountId: "" };
...
export function CodeProjectSettingsPage() {
  ...
  async function loadRuntimeProfile(repository: CodeRepository, requestedRole?: "frontend" | "backend") {
    ...
    const defaultRole = requestedRole ?? (repository.build_systems.includes("dotnet") || repository.languages.includes("C#") ? "backend" : repository.languages.includes("TypeScript/JavaScript") ? "frontend" : null);
    if (!defaultRole) { setRuntimeDraft(emptyRuntime); return; }
  ...
  async function openClone() {
    setCloneLines([]); setError(""); setCloneDraft({ projectId: selectedProjectId ?? projects[0]?.id ?? "", repositoryUrl: "", destinationDirectoryName: "", branch: "", gitAccountId: "" });
    try {
      const items = await listGitAccounts();
      const configuredAccounts = items.filter((account) => account.token_configured);
      setAccounts(configuredAccounts);
      setCloneDraft((item) => ({ ...item, gitAccountId: configuredAccounts.find((account) => account.is_active)?.id ?? configuredAccounts[0]?.id ?? "" }));
      setCloneOpen(true);
    } catch (value) { setError(message(value)); }
  }
  ...
  async function cloneRepository() {
    ...
    try {
      const event = await cloneCodeRepositoryViaWebSocket({ project_id: Number(cloneDraft.projectId), repository_url: cloneDraft.repositoryUrl.trim(), destination_directory_name: cloneDraft.destinationDirectoryName.trim() || undefined, branch: cloneDraft.branch.trim() || undefined, git_account_id: Number(cloneDraft.gitAccountId) }, (entry) => {
...
function RepositoryFormContent({ draft, projects, inspection, health, busy, editing, canPackage, selectedConfigs, onChange, onInspect, onBrowse, onBrowseFile, onSave, onDelete, onHealth, onOpenFile, onPackage }: { draft: RepositoryDraft; projects: CodeProject[]; inspection: CodeRepositoryInspection | null; health: CodeRepositoryHealth | null; busy: boolean; editing: boolean; canPackage: boolean; selectedConfigs: string[]; onChange: (value: RepositoryDraft) => void; onInspect: () => void; onBrowse: () => void; onBrowseFile: (target: "solution" | "configuration") => void; onSave: () => void; onDelete: () => void; onHealth: () => void; onOpenFile: (path: string) => void; onPackage: () => void }) {
  ...
  const managedRuntime = draft.languages.includes("C#") || frontendRepository;
  ...
  return <>
    ...
    <section className="mt-5 rounded-xl border border-slate-200 bg-slate-50/70 p-4">
      ...
      <p className="mt-1 text-xs text-slate-500">先识别目录；语言为单选。C# 请选择解决方案及 Web/API 启动工程，前端请选择 package.json；Java/Python 可登记 AI 上下文、配置文件与 Git 信息。</p>
      ...
      {draft.languages.includes("C#") && <FileSelection title="解决方案与工程文件" empty="请选择 .sln 解决方案；运行 API 时还需要选择可启动的 .csproj。" items={inspection?.solution_files ?? []} selected={draft.solutionFiles} onToggle={(value) => toggleFile("solutionFiles", value)} onBrowse={() => onBrowseFile("solution")}/>}
      ...
      {managedRuntime ? <div className="mt-5 grid gap-3 border-t border-slate-200 pt-4 md:grid-cols-2">
      ...
      </div> : <p className="mt-5 rounded-lg border border-amber-100 bg-amber-50 px-3 py-2 text-xs leading-5 text-amber-800">Java/Python 当前支持项目上下文、文件配置和 Git 差异查看；内置本地调试与打包目前仅适用于 C# 与 TypeScript/JavaScript。</p>}
    </section>
...
function CloneDialog({ projects, accounts, draft, lines, busy, onChange, onClose, onSubmit }: { projects: CodeProject[]; accounts: GitAccount[]; draft: CloneDraft; lines: ConsoleLine[]; busy: boolean; onChange: (value: CloneDraft) => void; onClose: () => void; onSubmit: () => void }) { return <Modal title="克隆远程代码库" onClose={onClose}><p className="text-xs leading-5 text-slate-500">可将同一远程仓库克隆到不同项目；每份代码库独立登记、运行和切换分支。</p><Field label="目标项目"><SearchableProjectSelect projects={projects} value={draft.projectId} onChange={(projectId) => onChange({ ...draft, projectId })} /></Field><Field label="HTTPS 仓库地址"><input className={input} value={draft.repositoryUrl} onChange={(event) => onChange({ ...draft, repositoryUrl: event.target.value })} placeholder="https://gitee.com/org/repository.git"/></Field><div className="grid gap-3 sm:grid-cols-2"><Field label="分支（可选）" hint="留空使用远程默认分支。"><input className={input} value={draft.branch} onChange={(event) => onChange({ ...draft, branch: event.target.value })} placeholder="develop"/></Field><Field label="本地目录名（可选）" hint="留空使用仓库名；不同分支可填写不同目录。"><input className={input} value={draft.destinationDirectoryName} onChange={(event) => onChange({ ...draft, destinationDirectoryName: event.target.value })} placeholder="guokun-aps-develop"/></Field></div><Field label="Git 账号"><select className={input} value={draft.gitAccountId} onChange={(event) => onChange({ ...draft, gitAccountId: event.target.value ? Number(event.target.value) : "" })}><option value="">选择已配置账号</option>{accounts.map((account) => <option key={account.id} value={account.id}>{account.provider} · {account.display_name} (@{account.username}){account.is_active ? "（当前默认）" : ""}</option>)}</select></Field>{accounts.length === 0 && <p className="mt-3 text-xs text-amber-700">尚未配置 Git 账号，请先前往 Git 管理添加访问令牌。</p>}{lines.length > 0 && <Console lines={lines}/>}<div className="mt-5 flex justify-end gap-2"><button onClick={onClose} disabled={busy} className="secondary-button">取消</button><button onClick={onSubmit} disabled={busy} className="primary-button">{busy && <Loader2 size={14} className="animate-spin"/>}开始克隆</button></div></Modal>; }

function SearchableProjectSelect({ projects, value, onChange }: { projects: CodeProject[]; value: number | ""; onChange: (value: number | "") => void }) {
  const [open, setOpen] = useState(false);
  const [query, setQuery] = useState("");
  const containerRef = useRef<HTMLDivElement>(null);
  const selected = projects.find((project) => project.id === value);
  const filteredProjects = projects.filter((project) => `${project.display_name} ${project.root_path}`.toLocaleLowerCase().includes(query.trim().toLocaleLowerCase()));
  ...
  function choose(projectId: number | "") {
    onChange(projectId);
    setQuery("");
    setOpen(false);
  }

  return <div ref={containerRef} className="relative mt-1.5">
    <button type="button" className={`${input} flex items-center justify-between text-left`} onClick={() => setOpen((current) => !current)} aria-haspopup="listbox" aria-expanded={open}>
      <span className={selected ? "text-slate-900" : "text-slate-400"}>{selected ? `${selected.display_name} · ${selected.root_path}` : "选择项目"}</span><ChevronDown size={16} className="shrink-0 text-slate-400" />
    </button>
    {open && <div className="absolute left-0 right-0 top-[calc(100%+4px)] z-20 overflow-hidden rounded-lg border border-slate-200 bg-white shadow-lg">
      <div className="border-b border-slate-100 p-2"><div className="relative"><Search size={15} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" /><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索项目名称或路径" className="h-9 w-full rounded-md border border-slate-200 pl-9 pr-3 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" /></div></div>
      <div className="max-h-56 overflow-y-auto py-1" role="listbox">
        <button type="button" role="option" aria-selected={value === ""} onClick={() => choose("")} className="w-full px-3 py-2 text-left text-sm text-slate-400 hover:bg-slate-50">选择项目</button>
        {filteredProjects.map((project) => <button type="button" role="option" aria-selected={project.id === value} key={project.id} onClick={() => choose(project.id)} className={`w-full px-3 py-2 text-left text-sm hover:bg-blue-50 ${project.id === value ? "bg-blue-50 text-blue-700" : "text-slate-700"}`}><span className="block truncate">{project.display_name}</span><span className="block truncate text-xs text-slate-400">{project.root_path}</span></button>)}
        {filteredProjects.length === 0 && <div className="px-3 py-4 text-center text-sm text-slate-400">没有匹配的项目</div>}
      </div>
    </div>}
  </div>;
}
...
function supportsManagedRuntime(repository: CodeRepository) { return repository.languages.includes("C#") || repository.languages.includes("TypeScript/JavaScript"); }
