...
export function apply(ctx, config) {
    ...
    const client = async () => {
        ...
        if (token === undefined)
            throw new Error('AiAgent is not signed in. Open Settings > Plugins > AiAgent Remote to sign in.');
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, execution) {
            ...
            if (context.length > config.maxContextChars)
                throw new Error(`Codex context exceeds ${config.maxContextChars} characters`);
