...
function TerminalPane({ id, onClosed }: { id: string; onClosed: () => void }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => void window.aiagent.terminalPoll(id).then(result => {
      if (result.data) terminal.write(result.data);
      if (result.ended) { terminal.writeln(`\r\n[${result.reason || 'Session ended.'}]`); window.clearInterval(timer); }
    }).catch(err => { terminal.writeln(`\r\n[${message(err)}]`); window.clearInterval(timer); }), 100);
    const resize = () => { const cols = Math.max(10, Math.floor((host.current?.clientWidth || 800) / 8.5)); const rows = Math.max(2, Math.floor((host.current?.clientHeight || 400) / 17)); terminal.resize(cols, rows); void window.aiagent.terminalResize(id, { cols, rows }).catch(() => undefined); };
    const observer = new ResizeObserver(resize); observer.observe(host.current!); resize();
    return () => { input.dispose(); observer.disconnect(); window.clearInterval(timer); terminal.dispose(); void window.aiagent.terminalClose(id).catch(() => undefined); };
  }, [id]);
...
function App() {
  ...
  const [prompt, setPrompt] = useState(''); const [answer, setAnswer] = useState(''); const [terminal, setTerminal] = useState('');
  ...
  const analyze = async () => { try { setStatus('Analyzing the explicitly shared excerpt…'); setAnswer((await window.aiagent.analyze({ prompt, context: content })).answer); setStatus('Analysis complete.'); } catch (err) { setStatus(message(err)); } };
