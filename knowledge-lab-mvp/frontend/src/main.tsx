...
function App() {
  ...
  useEffect(() => { void refresh(); const timer = window.setInterval(() => void refresh(), 1200); return () => window.clearInterval(timer); }, []);
  ...
  return <main>
    ...
    <div className="grid">
      <section className="panel import"><div className="panelHead"><div><p className="eyebrow">01 · INGEST</p><h2>添加学习资源</h2></div><span className="chip">异步索引</span></div>
        <form onSubmit={submitResource}><label>标题<input value={title} onChange={(event) => setTitle(event.target.value)} required /></label><label>来源<input value={source} onChange={(event) => setSource(event.target.value)} required /></label><label>正文<textarea value={content} onChange={(event) => setContent(event.target.value)} minLength={40} required /></label><button disabled={busy}>{busy ? "处理中…" : "写入并建立分层节点"}</button></form>
      </section>
